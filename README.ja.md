...
Eino が提供するもの：
- **[コンポーネント](https://github.com/cloudwego/eino-ext)**：`ChatModel`、`Tool`、`Retriever`、`ChatTemplate` などの再利用可能な構成要素。OpenAI、Ollama などの公式実装を用意しています
- **Agent Development Kit（ADK）**：ツール呼び出し、マルチエージェント連携、コンテキスト管理、Human-in-the-loop のための中断/再開、すぐに使えるエージェントパターンを備えた AI エージェント構築キット
- **オーケストレーション**：コンポーネントをグラフやワークフローとして接続し、単独で実行することも、エージェントのツールとして公開することもできます
- **[サンプル](https://github.com/cloudwego/eino-examples)**：よくあるパターンや実際のユースケースに対応した動作するコード
...
ChatModel を設定し、必要に応じてツールを追加すれば、エージェントが動きます：
...
実行フローを厳密に制御したい場合は、`compose` でグラフやワークフローを構築します：
...
構築したフローはエージェントのツールとして公開でき、決定的なワークフローと自律的な振る舞いを橋渡しします：
...
Eino はコンポーネントの抽象（ChatModel、Tool、Retriever、Embedding など）を定義し、OpenAI、Claude、Gemini、Ark、Ollama、Elasticsearch などの公式実装を提供しています。

→ [eino-ext](https://github.com/cloudwego/eino-ext)

## ストリーム処理

Eino はオーケストレーション全体でストリーミングを自動的に処理します。ノード間をデータが流れる際に、ストリームの連結、ボックス化、マージ、コピーを行います。コンポーネントは自身にとって意味のあるストリーミングパラダイムだけを実装すればよく、残りはフレームワークが処理します。

→ [ドキュメント](https://www.cloudwego.io/docs/eino/core_modules/chain_and_graph_orchestration/stream_programming_essentials/)

## コールバックアスペクト

コンポーネント、グラフ、エージェントを横断して、固定のポイント（OnStart、OnEnd、OnError、OnStartWithStreamInput、OnEndWithStreamOutput）でロギング、トレーシング、メトリクスを注入できます。

→ [ドキュメント](https://www.cloudwego.io/docs/eino/core_modules/chain_and_graph_orchestration/callback_manual/)

## 中断/再開

任意のエージェントやツールは、人間の入力を待つために実行を一時停止し、チェックポイントから再開できます。状態の永続化とルーティングはフレームワークが処理します。
...
- Eino（本リポジトリ）：型定義、ストリーミング機構、コンポーネントの抽象、オーケストレーション、エージェント実装、アスペクト機構

- [EinoExt](https://github.com/cloudwego/eino-ext)：コンポーネント実装、コールバックハンドラー、使用例、評価器、プロンプトオプティマイザー
...
本リポジトリでは `golangci-lint` を使用しています。ローカルでのチェックは次のとおりです：
...
本プロジェクトに潜在的なセキュリティ問題を発見した場合、または発見した可能性があると考えられる場合は、[セキュリティセンター](https://security.bytedance.com/src)または[脆弱性報告メール](mailto:sec@bytedance.com?subject=Feedback%20On%20Eino)を通じて Bytedance Security にご連絡ください。

公開の GitHub Issue は**作成しないで**ください。

## お問い合わせ
- メンバーシップ：[COMMUNITY MEMBERSHIP](https://github.com/cloudwego/community/blob/main/COMMUNITY_MEMBERSHIP.md)
- Issues：[Issues](https://github.com/cloudwego/eino/issues)
- Lark：[Feishu](https://www.feishu.cn/en/) で以下の QR コードをスキャンして、CloudWeGo/eino ユーザーグループに参加できます
